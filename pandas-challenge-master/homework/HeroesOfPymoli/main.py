...
def GetPurchasingAnalysis_Gender(maleGenders, femaleGenders, unkownGenders):
    #These create DFs containing all info based off each gender option.
    ...
    #These return the amount of purchases based off each gender.
    ...
    #These return the average purchase per person based off each gender.
    ...
    #These return the sum of all purchases based off each gender.
    ...
    #These return the average amount spent per person based off each gender.
...
def GetAgeDemographics(cleanUnqData):
    # The bins catch all corresponding values and map them to the binNames interface.
    bins = [0, 9, 14, 19, 24, 29, 34, 39, 100]
    binNames = ["<10", "10-14", "15-19", "20-24",
                "25-29", "30-34", "35-39", "40+"]
    ageDF = pd.DataFrame(data=cleanUnqData)
    #ageDF gets a new Total Count column inserted. Comprised of the data made from
    #binning the preexisting Age column.
    ...
    #A pseudo key is inserted to we can join it to the correct position later
    ageByBinDF.insert(1, "key", [0, 1, 2, 3, 4, 5, 6, 7])
    #pctData is an empty slice that cathes the age pct data
    pctData = []
    ...
    pct7 = pct7[2]
    pctData.append(pct7)
    # A new DF is created, and the slice of age pct data is converted to a series
    pctDataDF = pd.DataFrame()
    pctDataSeries = pd.Series(pctData)
    # The new series is inserted into the new DF
    pctDataDF.insert(0, "Percentage of Players", pctDataSeries)
    # The new DF is joined with the preexisting DF with the rest of the data
...
def Again():
    ...
    print(lineSep)
    print(lineSep + "\n")
    againANS = str(input(f"{color.BOLD}Would you like to generate a new report? {color.RED}[y/n]: {color.END}"))
    if againANS == "y":
        return "y"
    else:
        return None

def ChoicePrompt(PurchasingAnalysis_Total___Summary, GenderDemographics___Summary, maleGenders, femaleGenders, unknownGenders, cleanUnqData,  PurchasingAnalysis_Gender___Summary, AgeDemographics___Summary, bins, binNames, PurchasingAnalysis_Age___Summary, TopSpenders___Summary, MostPopularItems___Summary, rawTotals, mostPopularItemsDF, MostProfitableItems___Summary):
    print(
        f"{color.BOLD}{color.UNDERLINE}\nChoices of reports to generate:{color.END}\n\n• Purchasing Analysis (Total) Summary  {color.RED}[1]{color.END}")
    print(f"• Gender Demographics Summary          {color.RED}[2]{color.END}")
    print(
        f"• Purchasing Analysis (Gender) Summary {color.RED}[3]{color.END}")
    print(f"• Age Demographics Summary             {color.RED}[4]{color.END}")
    print(
        f"• Purchasing Analysis (Age) Summary    {color.RED}[5]{color.END}")
    print(f"• Top Spenders Summary                 {color.RED}[6]{color.END}")
    print(f"• Most Popular Items Summary           {color.RED}[7]{color.END}")
    print(f"• Most Profitable Items Summary        {color.RED}[8]{color.END}")
    print(
        f"• {color.BOLD}{color.UNDERLINE}ALL{color.END} of the above                     {color.RED}[9]{color.END}")
    print("\n")
    ans = int(input(f"{color.BOLD}Input your {color.RED}[#]{color.END} {color.BOLD}Choice:{color.END} "))
    print("")
    if ans == 1:
        print(lineSep)
        ...
        print(lineSep + "\n")
        print(AgeDemographics___Summary.to_string(index=True))
        time.sleep(1)
    elif ans == 5:
        print("\n" + lineSep)
        print(color.RED + '+**********************************| ' + color.END + color.BOLD +
        ...
        print(lineSep + "\n")
        print(MostProfitableItems___Summary.to_string(index=False))
    else: print(f"Your input of {ans} as not a valid option")
...
MostProfitableItems___Summary = GetMostProfitableItems(rawTotals, mostPopularItemsDF)

while Again() == "y":
    ChoicePrompt(PurchasingAnalysis_Total___Summary, GenderDemographics___Summary,
                 maleGenders, femaleGenders, unknownGenders, cleanUnqData, PurchasingAnalysis_Gender___Summary, AgeDemographics___Summary, bins, binNames, PurchasingAnalysis_Age___Summary, TopSpenders___Summary, MostPopularItems___Summary, rawTotals, mostPopularItemsDF, MostProfitableItems___Summary)



#* Three Observable Trends
#* 1) The vast majority of players are Male
#* 2) Nearly 2/3 of players are between 15 and 24 years old
#* 3) The age group of 20-24 accounts for nearly half of all revenue.
